...
export function buildRideICS(ride) {
  ...
  const description = [
    ...
    `Ride ${ride.position} of ${ride.total} in the Trailblu 2026 series.`,
  ...
  const lines = [
    ...
    `SUMMARY:${escapeICS(`Trailblu Ride: ${trail.name}`)}`,
    ...
    'TRIGGER:-P1D',
    'ACTION:DISPLAY',
    `DESCRIPTION:${escapeICS(`Trailblu ride tomorrow: ${trail.name}`)}`,
    'END:VALARM',
    'END:VEVENT',
    'END:VCALENDAR',
  ].filter(Boolean);
